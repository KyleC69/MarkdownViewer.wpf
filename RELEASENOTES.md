...
## 1.0.0 - 2026/05/02
  - Full test suite expansion: added 177 unit tests across all renderers and core components
  - New test coverage: `HeadingRenderer`, `ParagraphRenderer`, `EmphasisInlineRenderer`, `ListItemRenderer`, `BlockQuoteRenderer`, `ThematicBreakRenderer`, `TableRenderer`, `LiteralInlineRenderer`, `CodeInlineRenderer`, `LinkInlineRenderer`, `MarkdownEngine`, `RendererDispatcher`, `MarkdownView`
  - Promoted `MarkdownEngine.MergeThemeResources` from `private` to `internal` to enable direct unit testing
  - Tests conform to `Action_WhenCondition_ThenExpectedOutcome` naming, parameterized invariant coverage, and drift-detection standards
  - All tests verified passing on .NET 9.0-windows
