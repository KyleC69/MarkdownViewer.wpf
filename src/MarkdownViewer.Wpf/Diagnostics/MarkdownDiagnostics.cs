...
public static class MarkdownDiagnostics
{
    ...
    {
        Emit(new MarkdownDiagnosticEventArgs(MarkdownDiagnosticKind.HtmlInlineIgnored, $"Ignored unsupported inline HTML tag '{htmlTag}'."));
    }
    ...
    {
        Emit(new MarkdownDiagnosticEventArgs(MarkdownDiagnosticKind.ImageLoadFailed, $"Failed to load markdown image '{uri}'.", exception: exception));
    }
    ...
    {
        Emit(new MarkdownDiagnosticEventArgs(MarkdownDiagnosticKind.LinkNavigationFailed, $"Failed to navigate markdown link '{uri}'.", exception: exception));
    }
    ...
    {
        Emit(new MarkdownDiagnosticEventArgs(isBlock ? MarkdownDiagnosticKind.BlockRendered : MarkdownDiagnosticKind.InlineRendered, $"Rendered markdown {(isBlock ? "block" : "inline")} node {nodeType.FullName} in {duration.TotalMilliseconds:F3}ms.", nodeType: nodeType, duration: duration));
    }
    ...
    {
        var identity = $"{themeType.FullName}:{key}";
        lock (syncRoot)
        {
        ...
        Emit(new MarkdownDiagnosticEventArgs(MarkdownDiagnosticKind.ThemeKeyMissing, $"Theme '{themeType.FullName}' did not contain a resource for key '{key}'.", key: key));
    }
}